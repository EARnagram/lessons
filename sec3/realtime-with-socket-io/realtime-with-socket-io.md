...
- Config _socket.io_ on the Server

- Config  _socket.io_ in the Client (browser)
...
- **socket.io** is a JavaScript library that wraps the _websocket_ protocol and makes it easier to implement realtime bidirectional communication.
...
### Name some types of applications that take advantage of realtime communications
...
- `var circles` references a `<section>` that takes up most of the page.
...
- Remember, `io` represents the server and `socket` a client.
