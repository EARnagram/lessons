(function() {
  ...
  function TriumphsController($state, userDataService, $log, $http) {
    ...
    function getFishes() {
      $http.get('/api/fish').then(function(response) {
    ...
    function deleteFish(id) {
      $http.delete('/api/fish/' + id).then(function(response) {
    ...
    function updateFish(id) {
      $http.put('/api/fish/' + id, vm.editFish).then(function(response) {
        vm.editFish = {
          name: "",
          category: ""
        };
