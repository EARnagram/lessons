...
programs using models of real-world objects.

Virtually all OOP languages use what's known as a class-based approach where new
objects are created from classes that are used to construct new objects.
...
constructor functions that give us the ability to create a template for
countless similar, yet variable objects.
...
Arnold is so pumped about his new blackhole of an investment, he's going way
beyond your everyday machete:
...
But this doesn't really save us much (or any) time.

Plus, what is that `Object` constructor??

### Pause for Clarity

Before we can truly understand what's happening with JS constructor functions,
there are a couple key terms that we should cover.
...
objects is just a shortcut for calling:

``` javascript
var arnold = new Object();
```

So `arnold` was secretly created by the `Object` constructor function! We
simply added properties within it, much like adding the `puny` attribute or the
`machete` method.
...
For now, simply understand that using the `new` operator before a constructor
function is passing in a `{}` to bind to the variable aspects of the function.
Or, simpler yet, it's creating a new object out of the function.

### Back to Our Regularly Scheduled Programming...

Arnold's budget simply cannot accommodate taking this long to find actors.
...
Instead of opening up an object, and assigning it to an object literal, we'll be
creating them with a function.
...
`this` is a simple, yet frustratingly difficult keyword to fully grokk. We'll be
taking it in bite sizes throughout WDI.

In the case of the `Henchman` constructor function, `this` refers to the object
that is being created by the `new` operator. Therefore, you're placing the same
values directly on the new variable by simply calling:

`var bennet = new Henchman();`

However, `this` is much more powerful than it seems. Since we're assigning
...
What we've done is create a variable constructor to build henchmen with the same
...
that the average henchman will probably assign the concept of fear to Arnold in
their mind.
...
Assigning functions within a constructor function is actually bad form.

While there is only a single copy of the constructor function in memory, each
object being created takes up a little more memory. If each of these objects
have copies of a bunch of functions, it could add up.

By moving our object's methods to the _prototype_ of the constructor, there will
be only one copy of those functions, instead of one for each of the objects
created with the henchman constructor function!

Additionally, methods added to the prototype will be available to objects
created with the constructor function before the method was instantiated!

Putting functions on the __prototype__ of a constructor function instead of on
each object is considered best practice and it'll be what you see much more
often in the field (…until ES6 Classes take over).
...
1. Arnold needs a bunch of heroes for his new flick. Although he'll be doing
pretty much everything, it's always nice to have a bud.
  ...
  (look at the `arnold` object for guideance). Make heroes like Sylvester
  Stallone, Jan Michael Vincent and Jean-Claude van Damm from the same
  constructor function!

2. Turns out Arnold spent about half his money making a factory that produces
...
Inheritance is one of the most important aspects of programming. You can now
create templates for the repeated objects in your programs!
